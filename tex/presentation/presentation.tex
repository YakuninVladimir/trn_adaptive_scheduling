...
\vspace{2mm}
\smallnote{Пять строк --- как в диссертации (нет отдельного прогона \texttt{negative\_binomial} на WikiText).}
...
\vspace{2mm}
\smallnote{WikiText («до»): при отсутствии корневого \texttt{stopping\_eval.json} базовые столбцы проксируются из последнего полного прохода (см. \texttt{export\_wikitext\_lm\_tex\_tables.py}); для строки \texttt{finite\_discrete} --- из полного решётничного файла.}
\vspace{2mm}
\begin{block}{Вывод}
Дообучение головы оракула снижает среднюю глубину; на ARC часто сохраняется высокое качество (\texttt{arc\_stopping\_finetune\_compare.csv}).
\end{block}
\end{frame}
